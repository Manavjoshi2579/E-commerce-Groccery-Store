...
import { checkoutSummary, confirmReceivedBackend, fetchAdminOrder, fetchOrder, fetchOrders, fetchTracking, fetchDeliverySlots, requestReturnBackend } from "@/services/checkout";
...
function ProductsPage({ mode, value }: { mode?: string; value?: string }) {
  ...
  const renderFilterPanel = (mobile = false) => (
    <aside className={`${mobile ? "block h-full border-0 shadow-none" : "hidden lg:block lg:sticky lg:top-24"} overflow-hidden rounded-md border border-[#e8dfcd] bg-white shadow-[0_18px_50px_rgba(17,17,17,0.08)]`}>
      <div className={`${mobile ? "px-4 py-4" : "px-4 py-4"} flex items-center justify-between gap-3 border-b border-[#eadfca] bg-black text-white`}>
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#d4af37]">Refine shelf</p>
          <h3 className="display-font text-2xl font-black leading-none sm:text-xl">Filters</h3>
        </div>
        <button type="button" onClick={clearFilters} className="rounded-md border border-[#d4af37]/35 px-3 py-2 text-xs font-black uppercase text-[#f2d568]">Clear all</button>
      </div>
      <div className={`${mobile ? "max-h-none pb-2" : "max-h-[calc(100vh-160px)]"} grid gap-0 overflow-y-auto text-sm`}>
        ...
        <details open className="border-b border-[#eadfca] p-4">
          ...
          <div className="mt-3 grid gap-1">
            <label className="flex min-h-10 items-center gap-3 rounded-md px-2 py-2 hover:bg-[#faf4df]"><input type="radio" name={`category-${mobile}`} checked={!activeCategory} onChange={() => selectCategory("")} /> <span>All categories</span></label>
            {filterCategories.map((item) => <label key={item.slug} className="flex min-h-10 items-center gap-3 rounded-md px-2 py-2 hover:bg-[#faf4df]"><input type="radio" name={`category-${mobile}`} checked={activeCategory === item.slug || activeCategory === item.name} onChange={() => selectCategory(item.slug)} /> <span className="min-w-0 leading-snug">{item.name}</span></label>)}
          </div>
        </details>
        <details open className="border-b border-[#eadfca] p-4">
          ...
          <div className="mt-3 grid max-h-52 gap-2 overflow-y-auto pr-1">
            <label className="flex min-h-10 items-center gap-3 rounded-md px-2 py-2 hover:bg-[#faf4df]"><input type="radio" name={`brand-${mobile}`} checked={!brand} onChange={() => setBrand("")} /> <span>All brands</span></label>
            {filterBrands.map((item) => <label key={item.slug || item.id} className="flex min-h-10 items-center gap-3 rounded-md px-2 py-2 hover:bg-[#faf4df]"><input type="radio" name={`brand-${mobile}`} checked={brand === item.slug || brand === item.name} onChange={() => setBrand(item.slug || item.name)} /> <span className="min-w-0 leading-snug">{item.name}</span></label>)}
          </div>
        </details>
        <details open className="border-b border-[#eadfca] p-4">
  ...
  return (
    <CustomerShell>
      <main className="container-premium py-8">
        ...
        {mobileFiltersOpen && <div className="fixed inset-0 z-[70] flex items-end bg-black/65 p-2 pt-14 lg:hidden"><div className="mx-auto flex h-[min(88vh,760px)] w-full max-w-md flex-col overflow-hidden rounded-t-md border border-white/15 bg-[#f7f4ec] shadow-2xl"><div className="flex shrink-0 items-center justify-between border-b border-[#eadfca] bg-white px-4 py-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#9a7100]">Find faster</p><h2 className="display-font text-2xl font-black leading-none">Filters</h2></div><button type="button" onClick={() => setMobileFiltersOpen(false)} className="inline-flex h-10 w-10 items-center justify-center rounded-md border border-[#d8d1c2] text-black" aria-label="Close filters"><X size={20} /></button></div><div className="min-h-0 flex-1 overflow-y-auto">{renderFilterPanel(true)}</div><div className="shrink-0 border-t border-[#eadfca] bg-white p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]"><Button variant="gold" className="w-full" onClick={() => setMobileFiltersOpen(false)}>Show {list.length} products</Button></div></div></div>}
      </main>
    </CustomerShell>
  );
}
      ...
      <main className="container-premium py-8">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <section className="min-w-0 rounded-md bg-[#f4f6fb] p-4 sm:p-5">
            ...
            {step === 3 && <div><h2 className="display-font text-xl font-bold">Review Order</h2><div className="mt-4 grid gap-3 md:grid-cols-2"><PaymentCard active={payment === "COD"} title="Cash on Delivery" text={fulfillmentType === "PICKUP" ? "Pay at store pickup." : "Pay when your order arrives."} onClick={() => setPayment("COD")} /><PaymentCard active={false} disabled title="Online Payment" text="Online payment will start soon." onClick={() => toast("Online payment will start soon.", "info")}><p className="mt-3 rounded-md bg-[#fff8df] p-3 text-xs font-bold text-[#8a6500]">Coming soon</p></PaymentCard></div><p className="mt-5 text-sm text-black/60">{cart.length} items, {fulfillmentType === "PICKUP" ? "pickup from store" : `delivery ${deliveryDate} at ${slot}`}, payment {payment}</p>{address && <p className="mt-2 text-sm text-black/60">{fulfillmentType === "PICKUP" ? "Customer" : "Deliver to"} {address.name}, {address.line}, {address.city} - {address.pincode}</p>}<label className="mt-5 flex gap-2 text-sm"><input type="checkbox" checked={terms} onChange={(e) => setTerms(e.target.checked)} /> I agree to terms, easy cancellation, and freshness policy.</label><Button variant="gold" className="mt-5 w-full sm:w-auto" onClick={() => finish(true)} disabled={placing || deliveryDateIsPast}>{placing ? "Placing..." : "Place Order"}</Button></div>}
...
function AccountOrderCard({ order, products, coupons, onReturnSubmitted, onOrderChanged }: { order: Order; products: Product[]; coupons: StoreCoupons; onReturnSubmitted: () => Promise<void>; onOrderChanged?: () => Promise<void> }) {
  ...
  const confirmReceived = async () => {
    ...
    setConfirming(true);
    try {
      await confirmReceivedBackend(order.orderNumber, "Customer confirmed receipt");
      toast("Delivery confirmed. Thank you.", "success");
...
function AuthPage({ mode }: { mode: "login" | "signup" | "forgot-password" | "reset-password" }) {
  ...
  const showPasswordChecklist = (mode === "signup" || mode === "reset-password" || (mode === "forgot-password" && grant)) && !signupId;
  const safeNext = () => {
