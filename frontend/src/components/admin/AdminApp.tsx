      ...
      <aside className="sticky top-0 z-40 flex h-auto flex-col bg-black px-3 py-2 text-white shadow-xl lg:h-screen lg:p-4">
        ...
        <nav className={`${mobileNavOpen ? "grid" : "hidden"} fixed inset-x-0 bottom-0 top-[72px] z-40 content-start gap-1 overflow-y-auto border-t border-white/10 bg-[#080808] px-3 py-3 pb-8 lg:static lg:mt-0 lg:grid lg:gap-1 lg:overflow-visible lg:border-0 lg:bg-transparent lg:p-0`}>
          {visibleNav.map(([href, Icon, label]) => {
            ...
            return <Link key={href} href={`/admin${href ? `/${href}` : ""}`} onClick={() => setMobileNavOpen(false)} className={`flex min-h-12 items-center gap-3 rounded-md px-3 py-2.5 text-sm font-bold lg:min-h-0 lg:py-2 ${active ? "gold-gradient text-black" : "text-white/70 hover:bg-white/10 hover:text-white"}`}><Icon size={18} /><span className="min-w-0">{label}</span></Link>;
          })}
        </nav>
      </aside>
...
function KpiGroup({ title, items }: { title: string; items: (string | number)[][] }) {
  return <section className="premium-card overflow-hidden"><div className="border-b bg-white px-4 py-3"><h2 className="display-font text-sm font-black uppercase text-black/70">{title}</h2></div><div className="grid gap-2 p-3 sm:grid-cols-2 sm:gap-3">{items.map(([label, value, sub]) => <Stat key={String(label)} label={String(label)} value={String(value)} sub={String(sub)} />)}</div></section>;
}

function Stat({ label, value, sub }: { label: string; value: string; sub: string }) {
  return <div className="premium-card p-4 sm:p-5"><p className="text-[11px] font-bold uppercase text-black/50 sm:text-xs">{label}</p><h3 className="display-font mt-1 text-xl font-black sm:mt-2 sm:text-2xl">{value}</h3><p className="mt-1 text-xs text-[#8a6500]">{sub}</p></div>;
}

function Panel({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="premium-card overflow-hidden"><div className="border-b bg-white px-4 py-3 sm:px-5 sm:py-4"><h2 className="display-font text-base font-bold sm:text-lg">{title}</h2></div><div className="p-3 sm:p-5">{children}</div></section>;
}
...
function MetricBars({ data, unit = "number" }: { data: { label: string; value: number; display?: string }[]; unit?: "number" | "currency" }) {
  ...
  return <div><div className="mb-4 grid gap-2 sm:mb-5 sm:grid-cols-3 sm:gap-3"><Stat label="7 day revenue" value={unit === "currency" ? money(total) : String(total)} sub="Database orders" /><Stat label="Peak day" value={unit === "currency" ? money(max) : String(max)} sub="Highest in range" /><Stat label="Daily average" value={unit === "currency" ? money(total / Math.max(1, data.length)) : String(Math.round(total / Math.max(1, data.length)))} sub="7 day mean" /></div><div className="flex h-44 items-end gap-2 border-b border-black/20 pt-4 sm:h-64 sm:gap-3">{data.map((item) => { const height = Math.max(8, Math.round((item.value / max) * 100)); return <div key={item.label} className="flex h-full flex-1 flex-col justify-end gap-2"><div className="text-center text-[10px] font-bold text-black/65 sm:text-xs">{item.display || item.value}</div><div className="rounded-t bg-black transition-all" style={{ height: `${height}%` }} /><div className="text-center text-[10px] font-bold text-black/50 sm:text-xs">{item.label}</div></div>; })}</div></div>;
}
...
function DonutMetric({ rows }: { rows: { label: string; value: number }[] }) {
  ...
  return <div className="grid gap-4 md:grid-cols-[160px_1fr] md:items-center"><div className="mx-auto grid aspect-square w-full max-w-56 place-items-center rounded-full sm:max-w-64 md:max-w-none" style={{ background: `conic-gradient(${stops || "#e5e5e5 0% 100%"})` }}><div className="grid h-20 w-20 place-items-center rounded-full bg-white text-center sm:h-24 sm:w-24"><b className="display-font text-xl sm:text-2xl">{total}</b><span className="text-[11px] text-black/50 sm:text-xs">SKUs</span></div></div><div className="grid gap-2 sm:gap-3">{rows.map((row, index) => <div key={row.label} className="flex items-center justify-between gap-3 border-b pb-2"><span className="flex items-center gap-2 text-sm"><i className="h-3 w-3 shrink-0 rounded-full" style={{ background: colors[index % colors.length] }} />{row.label}</span><b>{row.value}</b></div>)}</div></div>;
}

function DashboardSplit({ stats }: { stats: [string, number][] }) {
  return <div className="grid grid-cols-2 gap-2 sm:gap-3">{stats.map(([label, value]) => <div key={label} className="rounded-md border border-[#eadfca] bg-white p-3 sm:p-4"><p className="text-[11px] font-bold uppercase text-black/45 sm:text-xs">{label}</p><h3 className="display-font mt-1 text-xl font-black sm:mt-2 sm:text-2xl">{value}</h3></div>)}</div>;
}
...
function ProductManager({ mode, id }: { mode?: "new" | "edit"; id?: string }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      fetchAdminProducts({ page: productPage, limit: pageSize, search: productSearch, imageStatus: imageStatusFilter || undefined })
        .then((result) => {
  ...
  }, [imageStatusFilter, mode, productPage, productSearch, replaceProducts, toast]);
  useEffect(() => {
  ...
  const refreshProducts = () => fetchAdminProducts({ page: productPage, limit: pageSize, search: productSearch, imageStatus: imageStatusFilter || undefined }).then((result) => {
    replaceProducts(result.products);
    if (result.pagination) setProductPagination(result.pagination);
  });
...
function Inventory({ productId, posOnly = false }: { productId?: string; posOnly?: boolean }) {
  ...
  const [offlineSalesPage, setOfflineSalesPage] = useState(1);
  const [offlineSalesPagination, setOfflineSalesPagination] = useState({ page: 1, pageSize: 5, total: 0, totalPages: 1 });
  ...
  const refreshPosMetrics = async () => {
    if (!posOnly) return;
    const [metrics, history] = await Promise.all([fetchAdminPosMetrics(pendingSyncCount), fetchAdminOfflineSales({ page: offlineSalesPage, pageSize: offlineSalesPagination.pageSize })]);
    setPosMetrics(metrics);
    setOfflineSales(history.sales || []);
    setOfflineSalesPagination(history.pagination || { page: offlineSalesPage, pageSize: 5, total: history.sales?.length || 0, totalPages: 1 });
  };
  useEffect(() => {
    refreshPosMetrics().catch(() => undefined);
  }, [posOnly, pendingSyncCount, offlineSalesPage]);
      ...
      {posOnly && <Panel title="Eagle Mart POS">
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_430px]">
          <div className="grid gap-3 self-start">
            <div className={`grid gap-3 rounded-md border p-3 text-sm font-bold sm:grid-cols-[1fr_auto] sm:items-center ${online ? "border-green-200 bg-green-50 text-green-800" : "border-red-200 bg-red-50 text-red-700"}`}><span>{online ? "Online: sales commit directly to server inventory." : "Offline Mode: sales are provisional and queued on this device."}</span><div className="grid grid-cols-2 gap-2 sm:flex"><Button className="min-h-11 px-3 text-xs sm:min-h-10" variant="outline" onClick={() => setScannerOpen(true)}>Scan</Button><Button className="min-h-11 px-3 text-xs sm:min-h-10" variant="outline" disabled={syncing || !queuedSales.some((sale) => ["QUEUED", "FAILED"].includes(sale.status))} onClick={syncQueued}>{syncing ? "Syncing..." : `Sync queue (${queuedSales.filter((sale) => sale.status !== "SYNCED").length})`}</Button></div></div>
            ...
            <div className="rounded-md border border-[#eadfca] bg-white shadow-sm">
              <div className="flex items-center justify-between gap-3 border-b border-[#eadfca] px-3 py-2"><h3 className="display-font text-base font-black">{offlineSearch.trim() ? "Search results" : "Quick select"}</h3><span className="text-xs font-bold uppercase text-black/45">{offlineSearch.trim() ? `${offlineMatches.length} matches` : `${quickRows.length} live SKUs`}</span></div>
              <div className="max-h-[520px] overflow-y-auto overscroll-contain p-2">
                {(offlineSearch.trim() ? offlineMatches : quickRows).length ? (offlineSearch.trim() ? offlineMatches : quickRows).map((row) => <button type="button" key={row.inventoryId} onClick={() => addOfflineItem(row)} className="grid min-h-20 w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-md border border-transparent px-3 py-2 text-left transition hover:border-[#d4af37] hover:bg-[#fff8df]"><span className="min-w-0"><b className="block truncate text-sm">{row.name}</b><span className="mt-1 block truncate text-xs text-black/50">{[row.sku, row.clientProductCode, row.barcode, row.pluCode, row.variantLabel || row.unit].filter(Boolean).join(" | ")}</span><span className="mt-1 inline-flex rounded-full bg-[#faf7ef] px-2 py-0.5 text-[11px] font-bold text-black/55">{row.stock} available</span></span><span className="text-right"><b className="block text-sm font-black">{money(row.price)}</b><span className="text-xs font-bold text-[#8a6500]">Add</span></span></button>) : <p className="p-4 text-sm text-black/55">No matching inventory item.</p>}
              </div>
            </div>
          </div>
...
function BillingInvoices() {
  ...
  useEffect(() => {
    pagedOrders.resetPage();
  }, [query, paymentStatus, paymentMethod, orderStatus, datePreset, exactDate]);
...
function AdminInvoiceView({ orderNumber }: { orderNumber?: string }) {
  ...
  const shouldAutoPrint = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("print") === "1";
  useEffect(() => {
    if (!orderNumber) {
      setLoading(false);
      return;
    ...
    fetchAdminOrder(orderNumber)
      .then(setOrder)
      .catch((error) => toast(error instanceof Error ? error.message : "Unable to load admin invoice.", "error"))
  ...
  if (loading) return <AdminShell section="invoices"><Panel title="Loading Invoice"><p className="text-sm text-black/60">Loading invoice from admin database...</p></Panel></AdminShell>;
  if (!order) return <AdminShell section="invoices"><Panel title="Invoice Not Found"><p className="text-sm text-black/60">This invoice could not be loaded from admin orders.</p><Link href="/admin/invoices"><Button className="mt-4" variant="outline">Back to invoices</Button></Link></Panel></AdminShell>;
  const rows = order.items.map((item, index) => {
    const product = products.find((entry) => entry.id === item.productId);
    const price = item.price ?? product?.price ?? 0;
    const gstRate = item.gst ?? product?.gst ?? 0;
    const lineTotal = item.lineTotal ?? price * item.qty;
    return { index: index + 1, name: item.name || product?.name || "Order item", sku: item.sku || product?.sku || "-", unit: item.unit || product?.unit || "-", qty: item.qty, price, tax: Math.round(price * item.qty * (gstRate / 100)), lineTotal };
  });
  const subtotal = order.subtotal ?? rows.reduce((sum, item) => sum + item.price * item.qty, 0);
  const discount = order.discount ?? 0;
  const couponDiscount = order.couponDiscount ?? 0;
  const gst = order.gstTotal ?? rows.reduce((sum, item) => sum + item.tax, 0);
...
function GenericAdmin({ section }: { section: string }) {
  ...
  const updateCatalogCategory = async (category: Category, patch: Partial<Category>) => {
    try {
      const saved = await updateAdminCategory(category.id, {
        name: patch.name ?? category.name,
        slug: patch.slug ?? category.slug,
        image: patch.image ?? patch.bannerImageUrl ?? category.bannerImageUrl ?? category.image,
        status: patch.active === false ? "INACTIVE" : "ACTIVE",
        sortOrder: patch.sortOrder ?? patch.displayOrder ?? category.sortOrder ?? category.displayOrder ?? 0,
        parentId: category.parentId ?? null,
      });
      setCatalogRows((items) => items.map((item) => item.id === saved.id ? saved : item));
      toast("Category banner settings saved", "success");
    } catch (error) {
      toast(error instanceof Error ? error.message : "Could not update category banner.", "error");
    }
  };
...
function AdminPageSwitch({ slug }: { slug: string[] }) {
  ...
  if (first === "invoices" && second) return <AdminInvoiceView orderNumber={second} />;
