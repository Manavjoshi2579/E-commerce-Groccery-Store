      ...
      {posOnly && <Panel title="Eagle Mart POS">
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_430px]">
          <div className="grid gap-3 self-start">
            <div className={`grid gap-3 rounded-md border p-3 text-sm font-bold sm:grid-cols-[1fr_auto] sm:items-center ${online ? "border-green-200 bg-green-50 text-green-800" : "border-red-200 bg-red-50 text-red-700"}`}><span>{online ? "Online: sales commit directly to server inventory." : "Offline Mode: sales are provisional and queued on this device."}</span><div className="grid grid-cols-2 gap-2 sm:flex"><Button variant="outline" onClick={() => setScannerOpen(true)}>Scan</Button><Button variant="outline" disabled={syncing || !queuedSales.some((sale) => ["QUEUED", "FAILED"].includes(sale.status))} onClick={syncQueued}>{syncing ? "Syncing..." : `Sync queue (${queuedSales.filter((sale) => sale.status !== "SYNCED").length})`}</Button></div></div>
          ...
          <div className="sticky bottom-3 grid gap-3 self-start rounded-md border border-[#eadfca] bg-white p-3 shadow-xl xl:top-24">
            ...
            <div className="max-h-[360px] overflow-y-auto overscroll-contain rounded-md border border-[#eadfca] bg-[#faf7ef] p-2">{offlineItems.length ? offlineItems.map(({ row, quantity }) => <div key={row.inventoryId} className="grid gap-2 border-b border-[#e0d2b4] bg-white px-2 py-2 last:border-b-0"><div className="grid grid-cols-[minmax(0,1fr)_auto] gap-3"><div className="min-w-0"><b className="block truncate text-sm">{row.name}</b><p className="truncate text-xs text-black/50">{row.sku} | {row.variantLabel || row.unit}</p></div><b className="text-sm">{money(row.price * quantity)}</b></div><div className="flex items-center justify-between gap-2"><div className="inline-flex h-10 overflow-hidden rounded-md border border-black"><button type="button" aria-label={`Decrease ${row.name}`} className="w-10 font-black" onClick={() => updateOfflineQty(row, quantity - 1)}>-</button><input aria-label={`Offline sale quantity for ${row.name}`} className="w-12 border-x text-center text-sm font-black" value={quantity} onChange={(event) => updateOfflineQty(row, Number(event.target.value || 0))} inputMode="numeric" /><button type="button" aria-label={`Increase ${row.name}`} className="w-10 font-black" onClick={() => updateOfflineQty(row, quantity + 1)}>+</button></div><button type="button" className="h-10 rounded-md border px-3 text-xs font-bold" onClick={() => updateOfflineQty(row, 0)}>Remove</button></div></div>) : <p className="rounded-md border border-dashed border-[#cfc4a6] bg-white p-4 text-sm text-black/55">Scan or choose products to start a POS bill.</p>}</div>
